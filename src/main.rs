...
fn main() {
	...
	let window_attributes = window::Window::default_attributes()
		...
		.with_inner_size(dpi::LogicalSize::new(32 * CELL_COLUMNS as u32, 45 + 32 * CELL_ROWS as u32));
...
fn app_logic(game: &mut Game) -> impl xilem::WidgetView<Game> {
	...
	let top_row = view::flex((
		view::sized_box::<Game, (), _>(view::label(format!("Mines: {}", MINE_COUNT - game.flag_count))).width(410.),
	...
	for y in 0..CELL_ROWS {
		...
		for x in 0..CELL_COLUMNS {
			let cell: Box<xilem::AnyWidgetView<_>> = match game.board[x][y] {
				Cell {status: CellStatus::Flagged, .. } => Box::new(view::button_any_pointer("!", move |game: &mut Game, button| match button {
					masonry::PointerButton::Secondary => game.flag(x, y),
					_ => (),
				})),
				Cell {status: CellStatus::Covered, .. } => match game.status {
					GameStatus::Playing => {
				  ...
				  GameStatus::Won | GameStatus::Lost => {
						Box::new(view::button(
							if game.board[x][y].value == CellValue::Mined { "X" } else { "" }
						, |_| {}))
					}
				}
				Cell {status: CellStatus::Revealed, value: CellValue::Mined} => Box::new(view::label(" X")),
